...
class LoraQModel(LoraModel):
    ...
    **Attributes**:
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if (
                    ...
                    kwargs.update(
                        {
                            "enable_gscales_decompose": target.enable_gscales_decompose,
                            "enable_svd_deltaW": target.enable_svd_deltaW,
                            "svd_deltaW_rank": target.svd_deltaW_rank,
                            "pack_bit": target.pack_bit,
                            "pack_dimension": target.pack_dimension,
                        }
                    )
                    new_module = QLinear(
                        target.in_features,
    ...
    def _replace_module(self, parent_module, child_name, new_module, old_module):
        ...
        if old_module.enable_gscales_decompose:
            new_module.step_scales = old_module.step_scales
            new_module.qstep = old_module.qstep
        if old_module.enable_svd_deltaW:
            new_module.svd_deltaW_A = old_module.svd_deltaW_A
            new_module.svd_deltaW_B = old_module.svd_deltaW_B
        if getattr(old_module, "fweight", None) is not None:
            new_module.fweight = old_module.fweight
...
def get_peft_qmodel(model, peft_config):
    ...
    assert (
        peft_config.task_type == "QLORA_CAUSAL_LM"
